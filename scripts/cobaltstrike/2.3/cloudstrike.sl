...
sub api_host_applet {
	...
	try {
		$hook = [new ServeApplet: cast($applet, 'b'), $windows, cast($java, 'b'), $title];
...
sub init_cloudstrike_hooks {
	...
	wait(fork({
		...
		global('%servers $webevents %idcache %cookie %ie %wm %js %vpn %tap %srv $dns %buffers %socks %bpivots %pipes %bartifacts %seen');
