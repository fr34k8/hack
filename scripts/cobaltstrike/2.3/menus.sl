...
sub view_items {
	...
	item($1, 'Applications', 'A', { thread(&createApplicationsBrowser); });
	item($1, 'Beacons', 'B', { thread(&createBeaconBrowser); });
	#item($1, 'Captured Data', 'C', { thread(&createFormBrowser); });
	...
	item($1, 'Web Log', 'W', { thread(&createWebLogTab); });
	...
	$t = menu($1, 'Reporting', 'R');
		item($t, 'Activity Report', 'A', { thread(&generateActivityReport); });
		item($t, 'Client Vuln. Report', 'C', { thread(&generateClientReport); });
		item($t, 'Hosts Report', 'H', { thread(&generateHostsReport); });
		item($t, 'Social Engineering' ,'S', { thread(&generateSocialReport); });
		item($t, 'Vulnerability Report', 'H', { thread(&generateVulnerabilityReport); });
...
sub armitage_items {
	...
	item($1, "Interfaces...", 'I', { listInterfaces(); });
	item($1, "Listeners...", 'L', { listListeners(); });
...
sub help_items {
	item($1, "Homepage", 'H', gotoURL("http://www.advancedpentest.com/")); 
	item($1, "Support", 'S', gotoURL("http://www.advancedpentest.com/support"));
	item($1, "Scripts", 'c', gotoURL("https://github.com/rsmudge/cortana-scripts"));

	if (isPaid()) {
		item($1, "Arsenal", 'r', gotoURL("http://www.advancedpentest.com/scripts?license=" . getLicenseKey()));
	}
	else {
		item($1, "Arsenal", 'r', {
			showError("The Cobalt Strike arsenal is a collection of Cortana scripts\nto assist licensed Cobalt Strike users with their engagements.\nThese features are only available to users with a valid license.");
		});
	}
	...
	separator($1);
	item($1, "About", 'A', {
		local('$dialog $handle $label $credits $scroll');
		...
		[$credits setEditable: 0];
		[$credits setFocusable: 0];
		[$credits setBorder: [BorderFactory createEmptyBorder: 5, 5, 5, 5]];
		[$credits setOpaque: 0];
		[$credits setLineWrap: 1];
		[$credits setWrapStyleWord: 1];

		$handle = [SleepUtils getIOHandle: resource("resources/about.html"), $null]; 
		[$label setText: readb($handle, -1)];
		closef($handle);

		$scroll = [new JScrollPane: $credits, [JScrollPane VERTICAL_SCROLLBAR_ALWAYS], [JScrollPane HORIZONTAL_SCROLLBAR_NEVER]];
		[$scroll setPreferredSize: [new Dimension: [$scroll getWidth], 100]];

		[$credits scrollRectToVisible: [new Rectangle: 0, 0, 1, 1]];
		[[$credits getCaret] setUpdatePolicy: [javax.swing.text.DefaultCaret NEVER_UPDATE]];
		$handle = [SleepUtils getIOHandle: resource("resources/credits.txt"), $null]; 
		[$credits setText: readb($handle, -1)];
		closef($handle);
