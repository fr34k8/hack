...
sub showScriptManager {
	local('$dialog $table $model $load $unload $reload $console $scripts $help');
	$dialog = [new JPanel];
	...
	[$help addActionListener: gotoURL("http://www.advancedpentest.com/help-scripting-cortana")];

	[$console addActionListener: &showScriptConsole];

	[$dialog add: center($load, $unload, $console, $scripts, $help), [BorderLayout SOUTH]];
