...
sub fixSessions {
	...
	foreach $session ($1) {
		...
		if ('opened_at' in $session) {
			$session['opened_at'] -= long($3);
		}

		if ('closed_at' in $session) {
			$session['closed_at'] -= long($3);
		}
	}
	return $1;
}
...
# 
sub queryData {
	local('%r $progress $adjust');	
	$adjust = call($1, "armitage.ping", ticks())['result'];

	# 1. extract the known vulnerability information
	%r['vulns'] = call($1, "db.vulns")["vulns"];
	...
	# 5. sessions...
	%r['sessions'] = fixSessions(call($1, "db.sessions")["sessions"], $3, $adjust);

	if ($progress) {
		[$progress setProgress: 36];
	}

	# 5.5 applications (Cloud Strike only)
	%r['applications'] = _profiles($1);

	# 5.6 web applications (Cloud Strike only)
	%r['weblog'] = values(_data_list($1, 'cloudstrike.weblog'));

	# 5.7 web keystroke data...
	%r['webkeys'] = form_submissions($1);

	# 5.8 spear-phishes sent...
	%r['spearphishes'] = values(_data_list($1, 'cloudstrike.sent_mail'));

	# 5.9 client-side vulnerabilities
	%r['client_vulns'] = clientVulns($1);

	# 6. timeline
	%r['timeline'] = fixTimeline(call($1, "db.events")['events'], $adjust, $3);
	...
	call($1, "armitage.prep_export", $2);

	$temp = call($1, "armitage.export_data");
...
sub _generateArtifacts {
	...
	%data = queryData($mclient, $1, $DESCRIBE, \$progress);
	...
	# 5.5 dump Cloud Strike data...
	dumpData("applications", @("external", "internal", "created_at", "application", "version", "token"), %data['applications']);
	dumpData("weblog", @("host", "created_at", "method", "uri", "response", "size", "handler", "user-agent", "token", "primary"), %data['weblog']);
	dumpData("webkeys", @("host", "created_at", "data", "site", "token"), %data['webkeys']);
	dumpData("spearphishes", @('to', 'to_name', 'server', 'status', 'time', 'token', 'template', 'subject', 'url', 'attachment'), %data['spearphishes']);
	dumpData("clientvulns", @('external', 'internal', 'created_at', 'application', 'version', 'name', 'refs', 'info', 'module'), %data['client_vulns']);
