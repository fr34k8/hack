...
sub bsleep {
	local('$3');
	if ($2 > 0) {
		call_async("beacon.task", $id, pack("IIII", 0x04, 8, $2 * 1000, $3));
		if ($3 == 0) {
			call_async("beacon.log_write", $id, "[*] Tasked beacon to sleep for $2 $+ s\n");
		}
		else {
			call_async("beacon.log_write", $id, "[*] Tasked beacon to sleep for $2 $+ s ( $+ $3 $+ % jitter)\n");
		}
	}
	else {
