...
global('$preferences $debug $motd $DATA_DIRECTORY $BASE_DIRECTORY $TITLE $CLIENT_CONFIG $c2profile');
...
sub loadPreferences {
	local('$file $prefs');
	$file = getFileProper(systemProperties()["user.home"], ".cobaltstrike.prop");
	...
	while (size(@ARGV) > 0) {
		...
		else if (@ARGV[0] eq "--profile") {
			local('$prof');
			$prof = @ARGV[1];
			@ARGV = sublist(@ARGV, 2);
			if (!-exists $prof) {
				print_error("$prof does not exist. Will use default profile");
			}
			else {
	        	        $c2profile = [c2profile.Loader LoadProfile: $prof];
				if ($c2profile is $null) {
					print_error("exiting because of errors in $prof $+ . Use ./c2lint to check the file");
					[System exit: 0];
				}
				else {
					print_good("I see you're into threat replication. $prof loaded.");
				}
			}
		}
	...
	$TITLE = [$prefs getProperty: "armitage.application_title.string", "Cobalt Strike"];
	return $prefs;
}
...
sub savePreferences {
	try {
		...
		if (-exists getFileParent($file)) {
			[$preferences save: [new java.io.FileOutputStream: $file], "Cobalt Strike Configuration"];
		}
	}
	catch $exception {
...
sub createPreferencesTab {
	...
	[$reset addActionListener: lambda({
		local('$file');
		$file = getFileProper(systemProperties()["user.home"], ".cloudstrike.prop");
		deleteFile($file);
		$preferences = loadPreferences();
		[$model clear: 256];
...
sub dataDirectory {
	...
	if ([$preferences getProperty: "armitage.log_data_here.folder", ""] eq "") {
		[$preferences setProperty: "armitage.log_data_here.folder", getFileProper(systemProperties()["user.home"], ".cobaltstrike")];
		savePreferences();
	}
