...
sub client {
	...
	if ($method ne "armitage.validate") {
		...
		else if ($app ne "armitage" && $app ne "cobaltstrike") {
			print_error("Rejected $eid (wrong application)");
			writeObject($handle, result(%(error => 1, message => "Your client is not compatible with this server.\nPlease use the latest version of Cobalt Strike.")));
			[[$handle getOutputStream] flush];
			return;
		}
		else if ($ver < 150405 && $app eq "cobaltstrike") {
			print_error("Rejected $eid (old software [ $+ $ver $+ ] - tell them to update!)");
		...
		if ($eid !is $null) {
			event("*** $eid joined\n");
			print_good("$eid joined from $c_address");
		}
		[[$handle getOutputStream] flush];
	}
	...
	while $temp (readObject($handle)) {
		...
		if ($method eq "session.meterpreter_read") {
			...
			if ($data ne "cleared the database") {
		...
		else if ($method eq "armitage.download_resource") {
			$file = getFileProper($DATA_DIRECTORY, $args[0]);
...
sub main {
	global('$client $mclient $MY_ADDRESS $MSFVERSION');
	local('$server %sessions $sess_lock $read_lock $lock_lock %locks %readq $id $events $error $auth %cache $cach_lock $client_cache $handle $console');
	...
	#
	if (size(@ARGV) < 5) {
		println("Cobalt Strike team server requires the following arguments:
	cobaltstrike --server host port user pass 
	...
	# setup cloudstrike specific stuff...
	init_cloudstrike_hooks(\$events);
	init_beacon_hooks();
...
service framework-postgres start");
	...
	# setup persistent listeners (grrlz love it)
	...
	while (1) {
		...
		if ($handle !is $null) {
			%readq[$id] = %();
			fork(&client, \$client, \$handle, \%sessions, \$read_lock, \$sess_lock, $queue => %readq[$id], \$id, \$events, \$auth, \%locks, \$lock_lock, \$cach_lock, \%cache, \$motd, \$client_cache, $c_address => [$server last], $_user => $user, $_pass => $pass, \$BASE_DIRECTORY, \$DATA_DIRECTORY);
