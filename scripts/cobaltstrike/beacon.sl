...
sub api_beacon_tab {
	...
	@commands = @('bypassuac', 'cd', 'checkin', 'clear', 'dllinject', 'download', 'execute', 'exit', 'getsystem', 'getuid', 'hashdump', 'help', 'inject', 'jobkill', 'jobs', 'kerberos_ticket_purge', 'kerberos_ticket_use', 'keylogger', 'kill', 'link', 'message', 'meterpreter', 'mimikatz', 'mode', 'powershell', 'powershell-import', 'ps', 'pwd', 'rev2self', 'runas', 'shell', 'sleep', 'socks', 'spawn', 'spawnto', 'steal_token', 'task', 'timestomp', 'unlink', 'upload', 'wdigest');
	($id, $data) = $2;
	...
	if ($data ismatch "(.*?)(\\s+)(.*)") {
		...
		if ($first eq "help") {
			...
			local('@mk');
			@mk = sorta(split(", ", "standard::, standard::answer, standard::coffee, standard::version, crypto::, crypto::providers, crypto::stores, crypto::certificates, crypto::keys, crypto::capi, crypto::cng, kerberos::, kerberos::ptt, kerberos::golden, kerberos::silver, kerberos::tgt, kerberos::list, kerberos::purge, lsadump::, lsadump::sam, lsadump::secrets, lsadump::cache, lsadump::lsa, process::list, sekurlsa::, sekurlsa::logonpasswords, sekurlsa::pth, sekurlsa::tickets, sekurlsa::ekeys, sekurlsa::dpapi, sekurlsa::minidump, sekurlsa::process, sekurlsa::searchpasswords, sekurlsa::msv, sekurlsa::wdigest, sekurlsa::kerberos, sekurlsa::tspkg, sekurlsa::livessp, sekurlsa::ssp, sekurlsa::credman, token::, token::list, token::whoami, token::elevate, token::revert, vault::, vault::list, vault::cred, event::, event::drop, event::clear, net::, net::user, net::localgroup, net::group, process::suspend, process::resume, process:stop, process::start, process::imports, process::exports, service::, service::start, service::remove, service::stop, service::suspend, service::resume, service::preshutdown, service::shutdown, service::list, ts::, ts::multirdp, minesweeper::, minesweeper::infos, misc::cmd, misc::regedit, misc::taskmgr, misc::, misc::ncroutemon, misc::detours, misc::wifi, misc::addsid, misc::memssp, misc::skeleton"));
			foreach $command (@mk) {
				if ("$first $+ $space $+ $second $+ *" iswm "$first $+ $space $+ $command") {
					push(@o, "$first $+ $space $+ $command");
				}
