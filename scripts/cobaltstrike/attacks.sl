	...
	foreach $host ($2) {
		foreach $port => $service (%hosts[$host]['services']) {
			...
			if ($port == 445 && "*Windows*" iswm getHostOS($host)) {
				...
				%options["psexec (psh)"] = lambda(&pass_the_hash_psh, $hosts => $2);
				%options["psexec (token)"] = lambda(&pass_the_token, $hosts => $2);
				%options["psexec (token+psh)"] = lambda(&pass_the_token_psh, $hosts => $2, $module => "windows/local/current_user_psexec_psh", $module => "windows/local/current_user_psexec", $title => "PsExec with User Token", $message => "psexec with session ");
			}
			else if ($port == 135 && "*Windows*" iswm getHostOS($host)) {
				%options["wmi (token+psh)"] = lambda(&pass_the_token_psh, $hosts => $2, $module => "windows/local/current_user_psexec_psh", $module => "windows/local/wmi", $title => "WMI", $message => "wmi login with session ");
			}
			else if ($name eq "ssh") {
				%options[$name] = lambda(&show_login_dialog, \$service, $hosts => $2);
				if ($MSFVERSION >= 41000) {
					%options["ssh (key)"] = lambda(&show_login_pubkey, $hosts => $2, $OPTION => "KEY_PATH");
				}
				else {
					%options["ssh (key)"] = lambda(&show_login_pubkey, $hosts => $2, $OPTION => "KEY_FILE");
				}
			}
			else if ($port == 902) {
...
sub addFileListener {
	...
	# let the user choose a covertvpn interface
	$actions["INTERFACE"] = {
		local('$tablef');
		$tablef = {
			refreshInterfaces($1, $2);
		};
		quickListDialog("Choose an interface", "Select", @("interface", "interface", "channel", "port", "mac", "client"), @(), $width => 640, $height => 240, lambda({
			[$call : $1];
		}, $call => $4), \$tablef);
	};
	...
	$actions["KEY_FILE"] = lambda(&keyfileHelper, \$model, $OPTION => "KEY_FILE");
	$actions["KEY_PATH"] = lambda(&keyfileHelper, \$model, $OPTION => "KEY_PATH");
