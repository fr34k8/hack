...
sub client {
	...
	if ($method ne "armitage.validate") {
		...
		else if ($ver < 140921 && $app eq "cobaltstrike") {
...
service framework-postgres start");
	...
	while (1) {
		...
		if ($handle !is $null) {
			%readq[$id] = %();
			fork(&client, \$client, \$handle, \%sessions, \$read_lock, \$sess_lock, $queue => %readq[$id], \$id, \$events, \$auth, \%locks, \$lock_lock, \$cach_lock, \%cache, \$motd, \$client_cache, $c_address => [$server last], $_user => $user, $_pass => $pass);
